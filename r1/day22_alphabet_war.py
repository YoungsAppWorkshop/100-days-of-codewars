...
Alphabet war - airstrike - letters massacre
    ...
    Introduction
        ...
        left side letters and right side letters was too high and the
        war began. The letters called airstrike to help them in war -
...
def alphabet_war(fight):
    scores = {'w': 4, 'p': 3, 'b': 2, 's': 1, 'm': -4, 'q': -3, 'd': -2, 'z': -1}  # noqa
    total, bombed = 0, re.sub('\w?\*+\w?', '', fight)
    for l in bombed:
        if scores.get(l):
            total += scores.get(l)
    if total > 0:
...
if __name__ == '__main__':
    print(alphabet_war("aa*aa"))
    print(alphabet_war("s*zz"))
    print(alphabet_war("*zd*qm*wp*bs*"))
    print(alphabet_war("zzzz*s*"))
