...
"""
    Rainfall - 6 kyu
        ...
        for months from January to December. The records of towns are separated
        by \n. The name of each town is followed by :.

        data and towns can be seen in "Your Test Cases:".

    Task:
        function: mean(town, string) should return the average of rainfall
            for the city town and the string data or data1
            (In R this function is called avg).

        function: variance(town, string) should return the variance of rainfall
            for the city town and the string data or data1.
...
Lima:Jan 1.2,Feb 0.9,Mar 0.7,Apr 0.4,May 0.6,Jun 1.8,Jul 4.4,Aug 3.1,Sep 3.3,Oct 1.7,Nov 0.5,Dec 0.7"""  # noqa
...
Lima:Jan 11.2,Feb 10.9,Mar 10.7,Apr 10.4,May 10.6,Jun 11.8,Jul 14.4,Aug 13.1,Sep 23.3,Oct 1.7,Nov 0.5,Dec 10.7"""  # noqa

towns = ["Rome", "London", "Paris", "NY", "Vancouver", "Sydney", "Bangkok",
        "Tokyo", "Beijing", "Lima", "Montevideo", "Caracas", "Madrid", "Berlin"]  # noqa
...
def parse(town, string):
    for line in string.split('\n'):
        if line.split(':')[0] == town:
            return line.split(':')[0], [float(y.split(' ')[1])
                                        for y in line.split(':')[1].split(',')]
    return None, None


def mean(town, string):
    city, data = parse(town, string)
    if city is None:
        return -1
    else:
        sum = 0
...
if __name__ == '__main__':
    ...
    print(mean("Lima", data))
    print(variance("Lima", data))
