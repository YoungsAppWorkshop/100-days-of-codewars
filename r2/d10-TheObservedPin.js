/**
  ...
  * possible in sense of: the observed PIN itself and all variations
  ...
  function getPINs (get_pins in python, GetPINs in C#).
  But please note that all PINs, the observed one and also the results, must
  be strings, because of potentially leading '0's. We already prepared some
  test cases for you.
...
const getPINs = observed => {
  ...
  let combinations = ['']
  for (const pin of observed) {
    // console.log(pin, adjacents[pin]);
    const arr = []
    for (const adjacent of adjacents[pin]) {
      // console.log(adjacent);
      for (const combination of combinations) {
        arr.push(combination + adjacent)
      }
    }
    combinations = arr
